...
export function validateLocation(lat, lng) {
	...
	if (!isInKano(lat, lng)) {
		...
		return {
			valid: false,
			error: `Location is ${distance.toFixed(
				1
			)}km outside Kano. Please drop a pin within Kano city.`
		}
	}
...
/**
 * Calculate distance to nearest Kano boundary in km
 */
export function getDistanceFromKano(lat, lng) {
	const clampedLat = Math.max(
		KANO_BOUNDS.minLat,
		Math.min(KANO_BOUNDS.maxLat, lat)
	)
	const clampedLng = Math.max(
		KANO_BOUNDS.minLng,
		Math.min(KANO_BOUNDS.maxLng, lng)
	)
