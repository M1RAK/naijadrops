...
function Step3Content() {
  ...
  async function createOrder() {
    ...
    try {
      ...
      // vendors row is guaranteed by the DB trigger at signup.
      // ensureVendorProfile handles the rare case where it's missing.
      const vendorProfile = await ensureVendorProfile(supabase, user.id)
    ...
    <div className='min-h-dvh bg-charcoal-950 flex flex-col'>
      ...
      <div className='flex items-center gap-4 px-5 pt-14 pb-5'>
        ...
        <div className='ml-auto flex gap-1.5'>
          {[1, 2, 3].map((s) => (
            <div key={s} className='h-1.5 w-6 rounded-full bg-emerald-500' />
          ))}
        </div>
      </div>
          ...
          <motion.div
            ...
            <motion.div
              ...
              <p className='text-charcoal-400 text-sm leading-relaxed mb-8'>
                Create a free account to confirm your delivery. Your route and pricing are saved.
              </p>
              <button
      ...
      {matchState === 'no_drivers' && (
        <div className='flex-1 flex flex-col items-center justify-center px-5 text-center'>
          ...
          <p className='text-charcoal-400 text-sm mb-8 leading-relaxed max-w-65'>
            No riders available right now. Please try again in a few minutes.
          </p>
          <button
            onClick={() => { setMatchState('idle'); setError(null) }}
