...
export default function LiveOrdersFeed({ initialOrders }) {
	...
	const handleForceCancel = async (orderId) => {
		if (
			!confirm(
				'CRITICAL ACTION: This will force-cancel the order and void the payment hold. Proceed?'
			)
		)
			return
	...
	const markers = orders.flatMap((order) => {
		...
		if (order.riders?.current_lat && order.riders?.current_lng) {
			list.push({
				lat: order.riders.current_lat,
				lng: order.riders.current_lng,
				color: 'emerald',
				type: 'rider'
			})
		}
		...
		<div className='grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-200px)]'>
			...
			<div className='flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar'>
				<div className='flex items-center justify-between px-2 mb-2'>
					<h2 className='text-[10px] font-black text-charcoal-500 uppercase tracking-[0.2em]'>
						Live Dispatches ({orders.length})
					</h2>
						...
						<motion.div
							...
							<div className='flex justify-between items-start'>
								<div className='space-y-1'>
									<div
										className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border inline-block ${
				...
				{orders.length === 0 && (
					<div className='py-20 text-center border-2 border-dashed border-white/5 rounded-3xl'>
						<MapIcon
							size={32}
							className='mx-auto mb-3 text-charcoal-800'
						/>
