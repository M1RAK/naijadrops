...
export default async function RiderLayout({ children }) {
	...
	// Fetch rider profile — we only need status and rejection_reason here
	...
	// FIX: Only redirect to onboarding if the rider row doesn't exist at all.
	// Previously this also redirected when users.name was missing, which caused
	// an infinite loop for approved riders who hadn't set their display name.
	if (!rider) {
	...
	return (
		<div className='flex flex-col min-h-dvh bg-charcoal-950 text-white selection:bg-emerald-500 overflow-x-hidden'>
			...
			<main className='flex-1 w-full max-w-lg mx-auto relative px-5 py-4'>
				{/* Pending toast — soft lock, they can still see the feed */}
				{isPending && (
				...
				{/* Approved content — render children normally */}
