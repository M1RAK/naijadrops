...
const STATUS_CONFIG = {
  pending: { label: "Searching", color: "text-amber-500", bg: "bg-amber-500/10 border-amber-500/20", icon: null },
  assigned: { label: "Rider Found", color: "text-blue-500", bg: "bg-blue-500/10 border-blue-500/20", icon: null },
  picked_up: { label: "Picked Up", color: "text-emerald-500", bg: "bg-emerald-500/10 border-emerald-500/20", icon: null },
  in_transit: { label: "In Transit", color: "text-emerald-500", bg: "bg-emerald-500/10 border-emerald-500/20", icon: null },
};
...
function MenuModal({ isOpen, onClose, onLogout, onProfile, userAvatar }) {
  ...
  return (
    <div className="fixed inset-0 z-110x items-start justify-end p-6">
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        className="relative w-full max-w-70 bg-charcoal-900 border border-white/10 rounded-4xl shadow-2xl overflow-hidden flex flex-col"
      >
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <span className="text-[10px] font-black text-charcoal-500 uppercase tracking-widest">Menu</span>
          <button onClick={onClose} className="p-2 text-charcoal-500 hover:text-white transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-4 space-y-2 overflow-y-auto flex-1">
          ...
          <button
            onClick={() => { router.push("/vendor/history"); onClose(); }}
            className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 text-white transition-all group"
          >
          ...
          <button
            onClick={() => { router.push("/driver/onboarding"); onClose(); }}
          ...
          <a
            href="mailto:yahaya.usama@naijadrops.tech"
            className="w-full flex items-center gap-4 p-4 rounded-2xl hover:bg-white/5 text-charcoal-300 transition-all"
          >
...
export default function DashboardPage() {
  ...
  async function loadData() {
    ...
    // 2. Get user profile — try 'name' column first, fall back gracefully
    ...
    if (profile?.name) {
      ...
      // No name set yet — open the profile modal so they can set one
      setIsProfileModalOpen(true);
    }

    // 3. FIX: Get the vendor profile to retrieve vendors.id (NOT users.id)
    // Orders use vendor_id which references vendors.id, not the auth user id
    ...
    if (!vendorProfile) {
      // Vendor profile doesn't exist yet — create it
    ...
    // 4. FIX: Query orders using vendors.id not users.id
    const ACTIVE_STATUSES = ["pending", "matched", "assigned", "picked_up", "in_transit"];

    const { data: orders } = await supabase
      .from("orders")
      .select("id, status")
      .eq("vendor_id", vendorProfile.id)  // ← correct: vendors.id
  ...
  return (
    <div className="h-dvh w-full relative overflow-hidden bg-charcoal-950">
          ...
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-charcoal-900 border border-white/10 overflow-hidden flex items-center justify-center shadow-xl">
              {avatarUrl ? (
                ...
                <UserIcon className="text-charcoal-600" size={20} />
              )}
            </div>
            <div>
      ...
      <div className="absolute bottom-0 inset-x-0 z-20">
        <div className="absolute inset-x-0 bottom-0 h-50 bg-linear-to-t from-charcoal-950 via-charcoal-950/95 to-transparent pointer-events-none" />
