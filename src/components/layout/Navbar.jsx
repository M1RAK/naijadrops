...
export default function Navbar() {
		...
		async function setupProfile() {
			try {
				...
				if (user) {
					...
					if (role === 'vendor' || !role) {
						// FIX: Get vendors.id first — orders use vendor_id which is vendors.id,
						...
						// Subscribe to changes on this vendor's orders
	...
	// Hide on ops-terminal and driver routes
	if (
		pathname?.startsWith('/ops-terminal') ||
	...
	return (
		<nav
			...
			<div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
				<div
					className={`relative flex justify-between h-16 sm:h-20 items-center px-6 rounded-4xl transition-all duration-500 ${
						scrolled
					...
					<Link
						href='/'
						className='shrink-0 flex items-center h-10 sm:h-12 group'>
								...
								<motion.div
									...
									<Link
										href={
											activeOrder.status ===
											'looking_for_driver'
												? `/send-package/step-3?orderId=${activeOrder.id}`
												: activeOrder.status ===
												  'awaiting_payment'
												? `/payment?orderId=${activeOrder.id}`
			...
			<AnimatePresence>
				{profile?.role === 'vendor' && activeOrder && (
					<motion.div
						...
						<Link
							href={
								activeOrder.status === 'looking_for_driver'
									? `/send-package/step-3?orderId=${activeOrder.id}`
